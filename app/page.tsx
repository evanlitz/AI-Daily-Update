import Link from 'next/link'
import db from '@/lib/db'
import { relTime } from '@/lib/utils'
import { StatCard, PageCard } from '@/components/HomeCards'
import { getTodaysBrief } from '@/lib/intelligence/brief'
import type { DailyBrief } from '@/lib/intelligence/brief'

// ── Page directory ────────────────────────────────────────────────────────────

const PAGE_DIRECTORY = [
  {
    href: '/feed', label: 'Feed', accent: '59,130,246', claude: true,
    desc: 'Real-time articles from 12+ sources — Anthropic blog, arXiv, HuggingFace, Hacker News, and AI-focused media. Claude screens each item for relevance and writes a one-sentence hook capturing the actual development. Updated at 8am and 8pm UTC.',
  },
  {
    href: '/digest', label: 'Digest', accent: '251,191,36', claude: true,
    desc: 'Weekly briefing generated by Claude from the past 7 days of articles and story updates. Covers macro trends, hot takes, research paper summaries, and a tools roundup — calibrated to what actually moved this week.',
  },
  {
    href: '/stories', label: 'Stories', accent: '167,139,250', claude: true,
    desc: 'Narrative threads that connect related news over time. Claude identifies when multiple articles point to the same development and groups them into a living story with title, significance level, and a weekly arc graph.',
  },
  {
    href: '/models', label: 'Models', accent: '52,211,153',
    desc: 'Release tracker for every major AI model — GPT, Claude, Gemini, Llama, and beyond. Shows benchmarks, context windows, pricing, and score charts over time. Includes a lab-by-lab view and a side-by-side comparison mode.',
  },
  {
    href: '/repos', label: 'Repos', accent: '96,165,250',
    desc: 'Trending GitHub repositories in AI/ML, ranked by star velocity. Tracks daily star gains, primary language, and last-updated timestamps — a quick read on what the open-source community is actually building.',
  },
  {
    href: '/datasets', label: 'Datasets', accent: '251,146,60',
    desc: 'HuggingFace and Kaggle datasets relevant to AI research, filterable by modality, task type, and popularity. Useful for finding training data and tracking what benchmarks the research community is working with.',
  },
  {
    href: '/predictions', label: 'Predictions', accent: '248,113,113',
    desc: 'A ledger of AI milestone predictions — capability thresholds, policy events, market moves — with confidence levels and evidence tracking. Status updates automatically when new evidence appears in the feed.',
  },
  {
    href: '/advisor', label: 'Advisor', accent: '167,139,250', claude: true,
    desc: 'Claude generates personalized AI project briefs based on trending developments and your skill level. Each mission includes a tech stack, difficulty rating, phase checklist, and skills breakdown.',
  },
  {
    href: '/timeline', label: 'Timeline', accent: '56,189,248',
    desc: 'Full-screen visualization of AI events and predictions on a historical axis. Browse from 2015 through projected 2030+ milestones, with confidence indicators and category color-coding.',
  },
] satisfies Array<{ href: string; label: string; accent: string; claude?: boolean; desc: string }>

// ── Constants ─────────────────────────────────────────────────────────────────

const CAT_COLOR: Record<string, { color: string; rgb: string }> = {
  capability: { color: '#a78bfa', rgb: '167,139,250' },
  safety:     { color: '#f87171', rgb: '248,113,113' },
  policy:     { color: '#fbbf24', rgb: '251,191,36'  },
  market:     { color: '#34d399', rgb: '52,211,153'  },
  tooling:    { color: '#60a5fa', rgb: '96,165,250'  },
  research:   { color: '#fb923c', rgb: '251,146,60'  },
}
const DEFAULT_CAT = { color: '#3b82f6', rgb: '59,130,246' }

const CONF_COLOR: Record<string, string> = {
  speculative: '#71717a',
  low:         '#60a5fa',
  medium:      '#fbbf24',
  high:        '#22c55e',
  confirmed:   '#3b82f6',
}

// ── Data fetching ─────────────────────────────────────────────────────────────

async function getHomeData() {
  const since24h = new Date(Date.now() - 24 * 3600_000).toISOString()
  const since6h  = new Date(Date.now() -  6 * 3600_000).toISOString()

  const [
    feedTodayRes,
    trendingRes,
    storiesRes,
    digestRes,
    modelCountRes,
    pipelineRes,
    predEvidenceRes,
    storyCountRes,
    predCountRes,
  ] = await Promise.all([
    db.execute({ sql: `SELECT COUNT(*) as count FROM feed_items WHERE fetched_at >= ?`, args: [since24h] }),
    db.execute({
      sql: `SELECT id, title, source, url, hook, velocity_score, published_at
            FROM feed_items WHERE velocity_score > 0
            ORDER BY velocity_score DESC LIMIT 6`,
      args: [],
    }),
    db.execute({
      sql: `WITH latest AS (
              SELECT thread_id, significance,
                     ROW_NUMBER() OVER (PARTITION BY thread_id ORDER BY created_at DESC) AS rn
              FROM story_events
            )
            SELECT st.id, st.title, st.category, st.last_updated, st.is_pinned,
                   l.significance as latest_sig
            FROM story_threads st
            LEFT JOIN latest l ON l.thread_id = st.id AND l.rn = 1
            LEFT JOIN user_affinity ua ON ua.category = st.category AND ua.source = 'story'
            WHERE st.status = 'active'
            ORDER BY st.is_pinned DESC, COALESCE(ua.open_count, 0) DESC, st.last_updated DESC
            LIMIT 5`,
      args: [],
    }),
    db.execute({ sql: `SELECT week_start, highlights, created_at FROM weekly_digest ORDER BY created_at DESC LIMIT 1`, args: [] }),
    db.execute({ sql: `SELECT COUNT(*) as count FROM ai_models`, args: [] }),
    db.execute({ sql: `SELECT MAX(fetched_at) as last_fetch, COUNT(*) as recent FROM feed_items WHERE fetched_at >= ?`, args: [since6h] }),
    db.execute({
      sql: `SELECT id, title, category, confidence, evidence, updated_at
            FROM ai_predictions
            WHERE status != 'past' AND evidence != '[]' AND updated_at >= ?
            ORDER BY updated_at DESC LIMIT 3`,
      args: [new Date(Date.now() - 7 * 24 * 3600_000).toISOString()],
    }),
    db.execute({ sql: `SELECT COUNT(*) as count FROM story_threads WHERE status = 'active'`, args: [] }),
    db.execute({ sql: `SELECT COUNT(*) as count FROM ai_predictions WHERE status != 'past'`, args: [] }),
  ])

  const feedToday    = (feedTodayRes.rows[0] as any)?.count ?? 0
  const trending     = trendingRes.rows as any[]
  const stories      = storiesRes.rows as any[]
  const modelCount   = (modelCountRes.rows[0] as any)?.count ?? 0
  const totalStories = (storyCountRes.rows[0] as any)?.count ?? 0
  const totalPreds   = (predCountRes.rows[0] as any)?.count ?? 0

  const digestRow = digestRes.rows[0] as any ?? null
  let highlights: string[] = []
  if (digestRow?.highlights) {
    try { highlights = JSON.parse(digestRow.highlights) } catch {}
  }

  const pipelineRow    = pipelineRes.rows[0] as any ?? null
  const lastFetch      = pipelineRow?.last_fetch as string | null
  const recentCount    = (pipelineRow?.recent as number) ?? 0
  const ageMs          = lastFetch ? Date.now() - new Date(lastFetch).getTime() : Infinity
  const freshnessColor = ageMs < 7 * 3600_000 ? '#34d399' : ageMs < 14 * 3600_000 ? '#fbbf24' : '#f87171'
  const freshnessLabel = ageMs < 7 * 3600_000 ? 'Fresh' : ageMs < 14 * 3600_000 ? 'Delayed' : 'Stale'

  const predEvidence = (predEvidenceRes.rows as any[]).map(r => {
    let latestEvidence: { title: string; url?: string } | null = null
    try {
      const arr = JSON.parse(r.evidence ?? '[]')
      latestEvidence = arr.length ? arr[arr.length - 1] : null
    } catch {}
    return { ...r, latestEvidence }
  })

  return {
    feedToday, trending, stories, digest: digestRow, highlights,
    modelCount, lastFetch, recentCount, freshnessColor, freshnessLabel,
    predEvidence, totalStories, totalPreds,
  }
}

// ── Helpers ───────────────────────────────────────────────────────────────────

function SectionDivider({ label, href, linkLabel }: { label: string; href?: string; linkLabel?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
      <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', color: '#71717a', textTransform: 'uppercase', flexShrink: 0 }}>
        {label}
      </p>
      <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.06)' }} />
      {href && linkLabel && (
        <Link href={href} style={{ fontSize: 12, color: '#71717a', textDecoration: 'none', fontWeight: 600, flexShrink: 0 }}>
          {linkLabel} →
        </Link>
      )}
    </div>
  )
}

// ── Page ──────────────────────────────────────────────────────────────────────

export default async function HomePage() {
  const [
    {
      feedToday, trending, stories, digest, highlights,
      modelCount, lastFetch, recentCount, freshnessColor, freshnessLabel,
      predEvidence, totalStories, totalPreds,
    },
    brief,
  ] = await Promise.all([getHomeData(), getTodaysBrief()])

  return (
    <main style={{ padding: '36px 48px', maxWidth: 1600, margin: '0 auto' }}>

      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <div style={{
        marginBottom: 36, paddingBottom: 36,
        borderBottom: '1px solid rgba(255,255,255,0.06)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 14 }}>
          <p style={{
            fontSize: 11, fontWeight: 700, letterSpacing: '0.16em',
            color: '#3b82f6', textTransform: 'uppercase',
          }}>
            Daily Intelligence Brief
          </p>
          <span style={{ width: 1, height: 12, background: 'rgba(255,255,255,0.1)', flexShrink: 0 }} />
          <p style={{ fontSize: 11, color: '#3f3f46', fontWeight: 600, letterSpacing: '0.05em' }}>
            {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
          </p>
        </div>
        <h1 style={{
          fontSize: 44, fontWeight: 900, color: '#f4f4f5',
          letterSpacing: '-0.03em', lineHeight: 1.05, marginBottom: 24,
        }}>
          AI Daily Update
        </h1>

        {brief ? (
          <div style={{ maxWidth: 680, marginBottom: 26 }}>
            {([
              { key: 'signal', label: 'Signal',  text: brief.signal },
              { key: 'rising', label: 'Rising',  text: brief.rising },
              { key: 'watch',  label: 'Watch',   text: brief.watch  },
              { key: 'shift',  label: 'Shift',   text: brief.shift  },
            ] as { key: keyof DailyBrief; label: string; text: string }[]).map(({ key, label, text }, i) => (
              <div key={key} style={{
                paddingTop:    i === 0 ? 0 : 16,
                paddingBottom: 16,
                borderBottom: i < 3 ? '1px solid rgba(255,255,255,0.05)' : 'none',
              }}>
                <span style={{
                  fontSize: 9, fontWeight: 900, letterSpacing: '0.16em',
                  color: '#3b82f6', textTransform: 'uppercase', display: 'block',
                  marginBottom: 6,
                }}>
                  {label}
                </span>
                <p style={{ fontSize: 15, color: '#a1a1aa', lineHeight: 1.75, margin: 0 }}>
                  {text}
                </p>
              </div>
            ))}
          </div>
        ) : (
          <p style={{
            fontSize: 16, color: '#a1a1aa', maxWidth: 620,
            lineHeight: 1.7, marginBottom: 26,
          }}>
            An automated intelligence system that synthesizes the AI landscape daily. Claude
            reads 12+ sources, separates signal from noise, threads narratives across time,
            and surfaces what actually matters — so you don&apos;t have to.
          </p>
        )}
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          {[
            { label: 'Powered by Claude Sonnet', color: '#a78bfa', rgb: '167,139,250' },
            { label: `${modelCount} models tracked`,      color: '#34d399', rgb: '52,211,153'  },
            { label: `${totalStories} active storylines`, color: '#60a5fa', rgb: '96,165,250'  },
            { label: `${totalPreds} predictions logged`,  color: '#fbbf24', rgb: '251,191,36'  },
          ].map(tag => (
            <span key={tag.label} style={{
              fontSize: 11, fontWeight: 700, letterSpacing: '0.05em',
              color: tag.color,
              background: `rgba(${tag.rgb},0.08)`,
              border: `1px solid rgba(${tag.rgb},0.2)`,
              borderRadius: 20, padding: '5px 12px',
            }}>
              {tag.label}
            </span>
          ))}
        </div>
      </div>

      {/* ── System status strip ──────────────────────────────────────────── */}
      <div style={{
        display: 'flex', alignItems: 'center', gap: 20, marginBottom: 20,
        padding: '9px 16px', borderRadius: 8,
        background: 'var(--surface)', border: '1px solid var(--border)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <div style={{ width: 7, height: 7, borderRadius: '50%', background: freshnessColor, flexShrink: 0 }} />
          <span style={{ fontSize: 11, fontWeight: 900, color: freshnessColor, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
            {freshnessLabel}
          </span>
        </div>
        <span style={{ width: 1, height: 14, background: 'rgba(255,255,255,0.08)', flexShrink: 0 }} />
        <span style={{ fontSize: 12, color: '#71717a' }}>
          Last fetch: <span style={{ color: '#a1a1aa', fontWeight: 700 }}>{lastFetch ? relTime(lastFetch) : 'never'}</span>
        </span>
        <span style={{ fontSize: 12, color: '#71717a' }}>
          Last 6h: <span style={{ color: '#a1a1aa', fontWeight: 700 }}>{recentCount} items</span>
        </span>
      </div>

      {/* ── Stat cards ───────────────────────────────────────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 14, marginBottom: 40 }}>
        <StatCard label="Signals Today"  value={feedToday}    sub="articles processed"      href="/feed"        accent="59,130,246"  />
        <StatCard label="Threads"        value={totalStories} sub="active storylines"        href="/stories"     accent="167,139,250" />
        <StatCard label="Models Tracked" value={modelCount}   sub="across all labs"         href="/models"      accent="52,211,153"  />
        <StatCard label="Weekly Brief"   value={digest ? '✓' : '—'} sub={digest ? `w/o ${digest.week_start}` : 'Not generated'} href="/digest" accent="251,191,36" />
      </div>

      {/* ── Today's Signals ──────────────────────────────────────────────── */}
      <div style={{ marginBottom: 36 }}>
        <SectionDivider label="Today's Signals" />
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: 20, alignItems: 'start' }}>

          {/* Trending feed */}
          <section style={{
            background: 'var(--surface)', border: '1px solid var(--border)',
            borderRadius: 12, overflow: 'hidden',
          }}>
            <div style={{
              padding: '14px 22px 12px',
              borderBottom: '1px solid var(--border)',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', color: '#71717a', textTransform: 'uppercase' }}>
                Signal Feed
              </p>
              <Link href="/feed" style={{ fontSize: 12, color: '#71717a', textDecoration: 'none', fontWeight: 600 }}>
                Full feed →
              </Link>
            </div>
            {trending.length === 0 ? (
              <p style={{ padding: '20px 22px', fontSize: 13, color: '#52525b', fontStyle: 'italic' }}>
                No signals yet — pipeline runs at 8am and 8pm UTC.
              </p>
            ) : (
              <div>
                {trending.map((item: any, i: number) => (
                  <a
                    key={item.id}
                    href={item.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      textDecoration: 'none', display: 'block',
                      padding: '14px 22px',
                      borderBottom: i < trending.length - 1 ? '1px solid var(--border)' : 'none',
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 5 }}>
                      <span style={{
                        fontSize: 9, fontWeight: 900, letterSpacing: '0.1em',
                        color: '#a1a1aa', background: 'rgba(255,255,255,0.05)',
                        border: '1px solid rgba(255,255,255,0.1)',
                        borderRadius: 3, padding: '1px 6px', textTransform: 'uppercase', flexShrink: 0,
                      }}>
                        {item.source.replace('rss:', '')}
                      </span>
                      {item.velocity_score > 1.5 && (
                        <span style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.1em', color: '#f87171', textTransform: 'uppercase' }}>
                          HOT
                        </span>
                      )}
                      <span style={{ fontSize: 10, color: '#52525b', marginLeft: 'auto', flexShrink: 0 }}>
                        {relTime(item.published_at)}
                      </span>
                    </div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: '#d4d4d8', lineHeight: 1.4, marginBottom: item.hook ? 4 : 0 }}>
                      {item.title}
                    </p>
                    {item.hook && (
                      <p style={{ fontSize: 12, color: '#71717a', lineHeight: 1.55 }}>
                        {item.hook.length > 130 ? `${item.hook.slice(0, 130)}…` : item.hook}
                      </p>
                    )}
                  </a>
                ))}
              </div>
            )}
          </section>

          {/* Active story threads */}
          <section style={{
            background: 'var(--surface)', border: '1px solid var(--border)',
            borderRadius: 12, overflow: 'hidden',
          }}>
            <div style={{
              padding: '14px 22px 12px',
              borderBottom: '1px solid var(--border)',
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            }}>
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', color: '#71717a', textTransform: 'uppercase' }}>
                Active Threads
              </p>
              <Link href="/stories" style={{ fontSize: 12, color: '#71717a', textDecoration: 'none', fontWeight: 600 }}>
                All threads →
              </Link>
            </div>
            {stories.length === 0 ? (
              <p style={{ padding: '20px 22px', fontSize: 13, color: '#52525b', fontStyle: 'italic' }}>
                No active threads — go to Stories and run an update.
              </p>
            ) : (
              <div>
                {stories.map((s: any, i: number) => {
                  const cat      = CAT_COLOR[s.category] ?? DEFAULT_CAT
                  const sigColors: Record<string, string> = { high: '#f87171', medium: '#fbbf24', low: '#52525b' }
                  const sigColor = sigColors[s.latest_sig] ?? '#52525b'
                  return (
                    <Link
                      key={s.id}
                      href="/stories"
                      style={{
                        textDecoration: 'none', display: 'block',
                        padding: '14px 22px',
                        borderBottom: i < stories.length - 1 ? '1px solid var(--border)' : 'none',
                      }}
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        <span style={{
                          fontSize: 9, fontWeight: 900, letterSpacing: '0.1em',
                          color: cat.color, background: `rgba(${cat.rgb},0.1)`,
                          border: `1px solid rgba(${cat.rgb},0.2)`,
                          borderRadius: 3, padding: '1px 6px', textTransform: 'uppercase', flexShrink: 0,
                        }}>
                          {s.category}
                        </span>
                        {s.latest_sig && (
                          <span style={{ width: 5, height: 5, borderRadius: '50%', background: sigColor, flexShrink: 0 }} />
                        )}
                        <span style={{ fontSize: 10, color: '#52525b', marginLeft: 'auto', flexShrink: 0 }}>
                          {relTime(s.last_updated)}
                        </span>
                      </div>
                      <p style={{ fontSize: 13, fontWeight: 700, color: '#d4d4d8', lineHeight: 1.4 }}>
                        {s.title}
                      </p>
                    </Link>
                  )
                })}
              </div>
            )}
          </section>
        </div>
      </div>

      {/* ── This Week's Digest ───────────────────────────────────────────── */}
      {digest && highlights.length > 0 && (
        <div style={{ marginBottom: 36 }}>
          <SectionDivider label="This Week's Digest" href="/digest" linkLabel="Read full" />
          <div style={{
            background: 'var(--surface)', border: '1px solid var(--border)',
            borderRadius: 12, padding: '20px 24px',
          }}>
            <p style={{ fontSize: 11, color: '#71717a', marginBottom: 16, fontWeight: 600 }}>
              Week of {digest.week_start} · {relTime(digest.created_at)}
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12 }}>
              {highlights.slice(0, 4).map((h: string, i: number) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  <div style={{
                    width: 6, height: 6, borderRadius: '50%',
                    background: '#3b82f6', flexShrink: 0, marginTop: 7,
                  }} />
                  <p style={{ fontSize: 14, color: '#a1a1aa', lineHeight: 1.65 }}>{h}</p>
                </div>
              ))}
            </div>
          </div>
        </div>
      )}

      {/* ── Signal Watch ─────────────────────────────────────────────────── */}
      {predEvidence.length > 0 && (
        <div style={{ marginBottom: 36 }}>
          <SectionDivider label="Signal Watch" href="/predictions" linkLabel="All predictions" />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
            {predEvidence.map((p: any) => {
              const confColor = CONF_COLOR[p.confidence] ?? '#71717a'
              const cat       = CAT_COLOR[p.category] ?? DEFAULT_CAT
              return (
                <Link key={p.id} href="/predictions" style={{ textDecoration: 'none' }}>
                  <div style={{
                    padding: '14px 18px',
                    background: 'var(--surface)', border: '1px solid var(--border)',
                    borderRadius: 10,
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <span style={{
                        fontSize: 9, fontWeight: 900, letterSpacing: '0.1em',
                        color: cat.color, background: `rgba(${cat.rgb},0.1)`,
                        border: `1px solid rgba(${cat.rgb},0.2)`,
                        borderRadius: 3, padding: '1px 6px', textTransform: 'uppercase',
                      }}>
                        {p.category}
                      </span>
                      <span style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.1em', color: confColor, textTransform: 'uppercase', marginLeft: 'auto' }}>
                        {p.confidence}
                      </span>
                    </div>
                    <p style={{ fontSize: 13, fontWeight: 700, color: '#d4d4d8', lineHeight: 1.4, marginBottom: 8 }}>
                      {p.title}
                    </p>
                    {p.latestEvidence && (
                      <p style={{ fontSize: 11, color: '#71717a', lineHeight: 1.5 }}>
                        Evidence: {p.latestEvidence.title?.slice(0, 80) ?? '—'}
                      </p>
                    )}
                    <p style={{ fontSize: 10, color: '#3f3f46', marginTop: 6 }}>
                      {relTime(p.updated_at)}
                    </p>
                  </div>
                </Link>
              )
            })}
          </div>
        </div>
      )}

      {/* ── Briefing Index ───────────────────────────────────────────────── */}
      <div>
        <SectionDivider label="Briefing Index" />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {PAGE_DIRECTORY.map(p => (
            <PageCard key={p.href} href={p.href} label={p.label} desc={p.desc} accent={p.accent} claude={p.claude} />
          ))}
        </div>
      </div>

    </main>
  )
}
